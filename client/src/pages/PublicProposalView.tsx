...
export default function PublicProposalView() {
  ...
  useEffect(() => {
    const user = getUser();
    // Check if user is logged in AND belongs to the same tenant as the proposal
    if (user && proposal && String(user.tenantId) === String(proposal.tenantId)) {
      setIsAdminView(true);
    } else {
  ...
  const canRespond = !isExpired && !isAccepted && !isRejected && !isAdminView;

  const headerGradient = headerStyle === 'gradient' 
    ? `linear-gradient(135deg, ${primaryColor} 0%, ${secondaryColor} 100%)`
    : primaryColor;

  const subtotal = proposal.pricingItems.reduce((sum, item) => sum + parseFloat(item.totalPrice || "0"), 0);
  const totalAmount = parseFloat(proposal.totalAmount || "0") || subtotal;
      ...
      {isAdminView && (
        <div className="bg-amber-50 border-b border-amber-200 py-2 px-4 text-center">
          <p className="text-sm font-medium text-amber-800">
            Preview Mode: You are viewing this as an administrator. Client actions are disabled.
          </p>
        </div>
      )}
      <div 
