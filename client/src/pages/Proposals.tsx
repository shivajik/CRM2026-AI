...
export default function Proposals() {
  ...
  const formatCurrency = (proposal: any) => {
    const amount = proposal.totalAmount;
    const currency = proposal.currency || "INR";
    
    // If totalAmount is missing or 0, check if we have pricing items or calculate from proposal data
    let displayAmount = amount ? parseFloat(amount) : 0;
    
    // Fallback if amount is 0/null but we have pricing items in the object
    // Note: The list API might not return pricingItems by default, 
    // but we can ensure the UI handles it gracefully
    if (displayAmount === 0 && proposal.pricingItems && Array.isArray(proposal.pricingItems)) {
      displayAmount = proposal.pricingItems.reduce((sum: number, item: any) => {
            ...
            <CardContent>
              <div className="text-2xl font-bold" data-testid="text-total-value">
                {new Intl.NumberFormat("en-US", {
                  style: "currency",
                  currency: "INR",
                }).format(parseFloat(analytics?.totalValue ?? "0"))}
              </div>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold" data-testid="text-avg-value">
                {new Intl.NumberFormat("en-US", {
                  style: "currency",
                  currency: "INR",
                }).format(parseFloat(analytics?.averageValue ?? "0"))}
              </div>
            </CardContent>
          </Card>
        </div>
